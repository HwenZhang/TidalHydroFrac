...
resultsname = 'results'
A_str = "{:.2f}".format(tide_amplitude)
A_str = A_str.replace('.','_')
casename = 'stokes_U09ma_L20000_A3e_27_tide1_00_relaxation'
path = []
...
if model == 'marine' and tides=='on':
    meshname = 'tides'+'_DX'+str(int(DX_s))+'_Lngth'+str(int(Lngth))+'.xdmf'
else:
    print ("Unknown mesh type!")
    exit()

mesh = Mesh('./meshes/tides_DX50.xml')
# XDMFFile(MPI.comm_world, "./meshes/"+meshname).read(mesh)
# with XDMFFile(MPI.comm_world, "./meshes/"+meshname) as meshfile:
#     meshfile.read(mesh)
