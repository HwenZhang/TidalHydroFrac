...
#-------------------------------------------------------------------------------
from params import tol,U0,model,Lngth,Hght,lake_radius
...
class SupraglacialLake(SubDomain):
    # Right boundary
    def inside(self, x, on_boundary):
        return (on_boundary and np.abs(x[0]-0.5*Lngth)<lake_radius and (x[1]>0.5*Hght))
