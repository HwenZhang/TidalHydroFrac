...
from params import t_final,nt_per_year,tides,model_setup,tide_amplitude,tides_modulation
...
def sl_change(t):
    if tides == 'on':
        if tides_modulation =='off':
            SLC = tide_amplitude*np.sin(4*np.pi*t/(3.154e7/12.0/30.0))  # tidal frequency of 2 per day
        elif tides_modulation =='on':
            SLC = tide_amplitude/2.498*np.sin(2.0*np.pi*t/(0.5*3.154e7/12.0/30.0))\
            +1.5/2.498*tide_amplitude*np.sin(2.0*np.pi*t*12.42/12/(0.5*3.154e7/12.0/30.0))   # tidal frequency of 2 per day
    else:
