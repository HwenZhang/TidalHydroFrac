...
from params import rho_i,lake_depth,g,tol,B,rm2_s,rm2,rho_w,C,eps_p,eps_v,g,\
        sea_level,dt,quad_degree,Lngth,U0,mu,eps_vs,test,bed_slope,newtonian,eta_const
...
def weak_form_marine(u,p,tau,u_old,p_old,tau_old,u_test,p_test,tau_test,f,g_base,g_in,g_out,s_in,ds,nu,T,newtonian,max_h):
    ...
    w2 = (-div(u_test)*p+inner(tau,grad(u_test)))*dx - inner(f, u_test)*dx\
         ...
         + Constant(rho_w*g*lake_depth)*dot(u_test,nu)*ds(5)
        #  + 0.0*dot(s_in, u_test)*ds(1)\
        #  + g_in*dot(nu, u_test)*ds(1)\

    # constitutive relation (upper-convected rate)
    # inner(max_h/(1e3*U0)*dot(u,nabla_grad(tau_test)),dot(u,nabla_grad(tau)))
...
def stokes_solve_marine(mesh,F_h,t,w_old):
        ...
        s_in = Expression(('0','ssin*rho_i*g*(h_in-x[1])*scos'),rho_i=rho_i, g=g,\
                h_in=h_in, ssin=np.sin(np.arctan(bed_slope)), scos=np.cos(np.arctan(bed_slope)),degree=1)
        ...
        # Solve for (u,p).
        Fw = weak_form_marine(u,p,tau,u_prev,p_prev,tau_prev,u_test,p_test,tau_test,f,g_base,g_in,g_out,s_in,ds,nu,T,newtonian,max_h)
