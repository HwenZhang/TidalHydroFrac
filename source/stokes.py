...
from params import rho_i,g,tol,B,rm2,rho_w,C,eps_p,eps_v,eps_v0,sea_level,dt,quad_degree,Lngth,U0,mu,lake_depth,lake_radius
...
def weak_form_marine(u,p,tau,u_old,p_old,tau_old,u_test,p_test,tau_test,f,g_base,g_out,lake_load,ds,nu,T,max_h):
    # Weak form of the marine ice sheet problem
    # viscosity (non-newtonian fluid)
    eta = B*(inner(sym(grad(u)),sym(grad(u)))+Constant(eps_v0))**(rm2/2.0)
    # strain rate
    ...
    w3 = inner(tau+lamda*((tau-tau_old)/dt+dot(u,nabla_grad(tau))-dot(grad(u),tau)-dot(tau, grad(u).T))\
         -2.*eta*epsilon,tau_test + dot(eta_stable*u,nabla_grad(tau_test)))* dx   
    Fw = w1 + w2 + w3
    return Fw
...
def stokes_solve_marine(mesh,F_h,t,w_old):
        ...
        w_previous = project(w_old,W) # used for compute \pdiff{\tau}{t}
        (u_prev,p_prev,tau_prev) = w_previous.split(True)
        ...
        lake_load = Expression('load*exp(-0.5*(x[0]-x_lake)*(x[0]-x_lake)/(0.1*radius*radius))',load=(rho_w-rho_i)*g*lake_depth,x_lake=0.5*Lngth,radius=lake_radius,degree=1)
        ...
        # Solve for (u,p).
        Fw = weak_form_marine(u,p,tau,u_prev,p_prev,tau_prev,u_test,p_test,tau_test,f,g_base,g_out,lake_load,ds,nu,T,max_h)
        ...
        newton_prm['error_on_nonconvergence'] = True #########!!!!! Dangerous
        ...
        eta = project(B*(inner(sym(grad(u)),sym(grad(u)))+Constant(eps_v0))**(rm2/2.0),eta_space)
        return w,P_res,eta
